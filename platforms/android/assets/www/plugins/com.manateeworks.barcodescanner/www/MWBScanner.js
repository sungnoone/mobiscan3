  			...
  			// Call the barcode scanner screen
  			 BarcodeScanner.MWBstartScanning(function(result)
  				...
  				if (result && result.code){
              		navigator.notification.alert(result.code, function(){

              		}, result.type, 'Go');
                      var obj = {barcode:result.code,user:'t00277',pwd:'1234'};
                      var params = $.base64.encode($.param(obj));
                      var encodeStr = 'http://192.168.1.237/index.html?str='+params;
                      //var str1 = encodeURIComponent(encodeStr);

              		//alert("http://www.google.com.tw?"+paramStr);
              		alert(encodeStr);
              		window.location = encodeStr;
  				}
